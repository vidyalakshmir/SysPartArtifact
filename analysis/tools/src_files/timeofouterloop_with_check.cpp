...
const string *find_current_function(INS ins)
{
    RTN cur_fn = INS_Rtn(ins);
    
    if(RTN_Valid(cur_fn))
    {
        string name = RTN_Name(cur_fn);
...
ADDRINT find_function_address(INS ins)
{
    RTN cur_fn = INS_Rtn(ins);
    
    if(RTN_Valid(cur_fn))
    {
        return RTN_Address(cur_fn);
    }
    else
    {
...
VOID  do_call_indirect(ADDRINT target, BOOL taken, ADDRINT ip, const string *f, THREADID threadid)
{
	//thread_data_t* tdata = get_tls(threadid);
    if( !taken ) 
    {
        //TraceFile << ip << " Indirect Branch/call not taken "<< target << endl << endl;
        return;
    }

    const string *s = Target2String(target);

    //(tdata->threadFile) << ip << " " << *f << " " << target << " " << *s << " icall" << endl;
...
// This check ensures that if a function returns from within a loop, then you add that loop to executed loops and that  current loop is set to NULL
VOID Before(const string *f, THREADID threadid, set<loop*>* loop_vec)
{
	thread_data_t* tdata = get_tls(threadid);

    if(tdata->cur_loop == NULL)
    	return;
    if(tdata->cur_loop->func_name == *f)
    {
		OS_Time(&tdata->cur_loop->end_time);
		UINT64 temp_diff = tdata->cur_loop->end_time - tdata->cur_loop->start_time;
		pair<UINT64,UINT64> temp_pair(temp_diff, tdata->cur_loop->iterations);
...
VOID Instruction(INS ins, VOID *v)
{
	...
	RTN routine = RTN_FindByAddress(instr_address);
	string rtn_name="";
	if(RTN_Valid(routine))
	{
			*OutFile<<"Routine is "<<RTN_Name(routine)<<" "<<std::hex<<instr_address<<endl;
		rtn_name = RTN_Name(routine);
	}
	IMG img1 = IMG_FindByAddress(instr_address);
	const string *f = find_current_function(ins);
	...
	if(IMG_Valid(img1))
	{
		...
		{
			offset = instr_address - img_addr;
			*OutFile << "EEEE " << im_name << " " <<std::hex<<offset<<" "<<instr_address<<" "<<img_addr<<endl;
		...
		if(iter != entry_map.end())
		{
			...
			if((im_name == l->image_name) && (*f == l->func_name))
			{
	...
	if( INS_IsRet(ins) )
                {
                    ...
                    INS_InsertCall( ins, IPOINT_BEFORE, (AFUNPTR)Before,
                                   IARG_PTR,f, IARG_THREAD_ID, IARG_END);
                }
}
